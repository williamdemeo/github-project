"""
File: scripts/_utils/pipeline_types.py

Description: Functional programming primitives and immutable data structures.

  This module provides functional error handling (Result), structured
  errors (PipelineError), and immutable pipeline-state types.
...
Provenance:
  ...
  items 1 and 2 (unwrap/unwrap_or on Ok(None); map() no longer swallows
  exceptions).  Downstream projects re-vendor from here — see
  scripts/VERSION and the README's upgrade section.
"""
...
class Result(Generic[T, E]):
    ...
    def unwrap(self) -> T:
        ...
        Keys solely off _is_ok: Result.ok(None) is a legal and common
        construction (write_text returns Result[None, _]), so the
        contained value must never influence the ok/err decision.
        """
        if self._is_ok:
    ...
    def unwrap_or(self, default: T) -> T:
        ...
        Ok(None) returns None — the contained value — not the default.
        """
    ...
    def map(self, f: Callable[[T], B]) -> Result[B, E]:
        ...
        Exceptions raised by f propagate: a raising f is a bug in the
        caller, not an expected failure, and catching it here would
        smuggle a bare Exception into the declared error type E.
        """
        if self._is_ok:
